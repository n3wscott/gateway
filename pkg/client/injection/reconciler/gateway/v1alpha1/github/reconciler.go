...
// controller reconciling v1alpha1.GitHub.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.GitHub. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.GitHub) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.GitHub.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.GitHub. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.GitHub) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.GitHub resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister gatewayv1alpha1.GitHubLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister gatewayv1alpha1.GitHubLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.GitHubs(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.GitHub, desired *v1alpha1.GitHub) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.GatewayV1alpha1().GitHubs(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.GatewayV1alpha1().GitHubs(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.GitHub) (*v1alpha1.GitHub, error) {
	finalizerName := defaultFinalizerName

	actual, err := r.Lister.GitHubs(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.GatewayV1alpha1().GitHubs(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.GitHub) (*v1alpha1.GitHub, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.GitHub, reconcileEvent reconciler.Event) (*v1alpha1.GitHub, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
