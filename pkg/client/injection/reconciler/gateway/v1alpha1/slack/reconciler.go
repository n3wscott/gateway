...
// controller reconciling v1alpha1.Slack.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Slack. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties.
	ReconcileKind(ctx context.Context, o *v1alpha1.Slack) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.Slack resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister gatewayv1alpha1.SlackLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister gatewayv1alpha1.SlackLister, recorder record.EventRecorder, r Interface) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.Slacks(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.Slack, desired *v1alpha1.Slack) error {
	existing = existing.DeepCopy()
	return RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.GatewayV1alpha1().Slacks(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.GatewayV1alpha1().Slacks(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
